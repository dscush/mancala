...
function AlphaBetaAI(player) {
    this.times = [];
    this.player = player;
    this.playMove = function(boardModel, depth=0, alpha=-Infinity, beta=Infinity) {
        if (depth === 0) {
        ...
        for (var i = 0; i < boardModel.boardLength / 2 - 1; i++) {
            ...
            if (boardModel.board[move] !== 0) {
                ...
                nextBoard.playMove(move);
                try {
                    moveValues[i] = this.playMove(nextBoard, depth + 1, alpha, beta);
                } catch (err) {
                    console.log(err);
                    console.log(nextBoard);
                    console.log("playing " + move);
                ...
                if (boardModel.playerTurn == this.player && moveValues[i] > alpha) { // MAX
                    alpha = moveValues[i];
                } else if (boardModel.playerTurn !== this.player && moveValues[i] < beta){ // MIN
                    beta = moveValues[i];
                }
                if (alpha >= beta) {
                    break;
